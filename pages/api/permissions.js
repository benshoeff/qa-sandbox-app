...
export default async function handler(req, res) {
  ...
  switch (method) {
    case 'GET':
      if (id) {
        const item = await getById('permissions', id)
        if (!item) return res.status(404).json({ error: 'Permission not found' })
        return res.status(200).json(item)
      }
      return res.status(200).json(await getAll('permissions'))
    ...
    case 'POST':
      ...
      const created = await create('permissions', { name, resource, action, description: description || '' })
      return res.status(201).json(created)

    case 'PUT':
      if (!id) return res.status(400).json({ error: 'id is required' })
      const updated = await update('permissions', id, req.body)
      if (!updated) return res.status(404).json({ error: 'Permission not found' })
      return res.status(200).json(updated)

    case 'DELETE':
      if (!id) return res.status(400).json({ error: 'id is required' })
      const deleted = await remove('permissions', id)
